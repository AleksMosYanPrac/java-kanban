...
public final class HistoryHandler extends BaseHttpHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        switch (action) {
            case GET_HISTORY -> getHistory(exchange, 200);
            default -> super.writeBadRequest(exchange);
        }
    }

    private void getHistory(HttpExchange exchange, int code) throws IOException {
        super.writeSuccess(exchange, responseConverter.convert(taskManager.getHistoryOfViewedTasks()), code);
    }
